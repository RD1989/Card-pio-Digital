...
export const CartBar = ({ whatsappNumber, restaurantSlug, isClosed, nextOpening }: CartBarProps) => {
  ...
  const generateWhatsAppLink = async () => {
    ...
    try {
      // Registrar o pedido no Supabase se necessário futuramente
      // console.log('Pedido registrado analiticamente');
    } catch (error: any) {
        ...
        >
          <div className="glass-card p-4 rounded-3xl flex items-center justify-between shadow-2xl backdrop-blur-xl border border-white/10">
            ...
            <button
              ...
              className={`px-6 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all active:scale-95 disabled:opacity-70 ${isClosed ? 'grayscale pointer-events-none' : ''}`}
            ...
            >
              {loading ? (
                ...
                <>Ver Pedido <ChevronRight className="w-5 h-5" /></>
              )}
            </button>
          </div>
        </motion.div>
