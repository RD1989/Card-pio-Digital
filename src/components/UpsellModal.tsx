...
export const UpsellModal = ({ isOpen, onClose, accentColor, upsellProducts }: Props) => {
  ...
  const upsellItems = upsellProducts.filter(p => p.is_upsell && p.is_available).slice(0, 2);
                ...
                {upsellItems.map((item) => (
                  ...
                  >
                    <div className="w-16 h-16 rounded-2xl overflow-hidden shrink-0 border border-zinc-800">
                      <img src={item.image_url || (item as any).image} className="w-full h-full object-cover" alt={item.name} />
                    </div>
